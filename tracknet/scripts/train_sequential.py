...
Examples:
  ...
  uv run python -m tracknet.scripts.train_sequential training.optimizer.lr=1e-4
    ...
    parser.add_argument(
        "--data", 
        default="tracknet", 
        help="Data configuration name (default: tracknet)"
    )
    parser.add_argument(
        "--training", 
        default="default", 
    ...
    parser.add_argument(
        "--batch-size",
        type=int,
        help="Batch size (overrides training.batch_size)"
    )
    ...
    parser.add_argument(
        "--output-dir",
        help="Output directory (overrides training.output_dir)"
    )
...
def main(argv: List[str] | None = None) -> int:
    ...
    for model, result in results.items():
        ...
        print(f"{status} {model}: {result['duration']:.2f}s (exit: {result['exit_code']})")
