...
sys.path.append(
    os.path.join(os.path.dirname(__file__), "../../../third_party/Deformable-DETR")
)
...
class DeformableFPNDecoder(nn.Module):
    ...
    def __init__(self, cfg: DeformableDecoderConfig):
        ...
        self.level_embed = nn.Parameter(
            torch.Tensor(cfg.num_feature_levels, cfg.d_model)
        )
        ...
        self.encoder = DeformableTransformerEncoder(
            encoder_layer, cfg.num_encoder_layers
        )
        ...
        self.fpn = FeaturePyramidNetwork(
            [cfg.lateral_dim] * len(cfg.in_channels), cfg.lateral_dim
        )
    ...
    def forward(self, feats: list[torch.Tensor]) -> torch.Tensor:
        ...
        proj_feats = [
            proj(feat) for proj, feat in zip(self.input_proj, feats, strict=False)
        ]
        ...
        for i in range(len(proj_feats)):
            level_embed = (
                self.level_embed[i]
                .view(1, 1, -1)
                .expand(spatial_shapes[i, 0] * spatial_shapes[i, 1], -1, -1)
            )
        ...
        fpn_inputs = [self.output_proj(feat) for feat in encoded_feats]
...
if __name__ == "__main__":
    ...
    feats = [
        torch.randn(B, c, h, w)
        for c, (h, w) in zip(in_channels, spatial_sizes, strict=False)
    ]
