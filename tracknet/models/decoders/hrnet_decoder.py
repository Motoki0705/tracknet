...
class ExchangeUnit(nn.Module):
    ...
    def forward(self, xs: list[torch.Tensor]) -> list[torch.Tensor]:
        # 各分岐で軽く整形
        xs = [op(x) for op, x in zip(self.branch_ops, xs, strict=False)]
        # 融合
        ...
        for k, fuse_k in enumerate(self.fuse_layers):
            ...
            for i, trans in enumerate(fuse_k):
                ...
                if i > k:
                    ...
                    z = F.interpolate(
                        z, size=target_size, mode="bilinear", align_corners=False
                    )
...
class HRDecoderConfig:
    ...
    out_channels: int | None = None  # 最後に 3x3 で出力チャネルへ変換（省略可）
...
class HRDecoder(nn.Module):
    ...
    def __init__(self, cfg: HRDecoderConfig):
        ...
        self.lateral = nn.ModuleList(
            [
                conv1x1_bn_relu(cin, w)
                for cin, w in zip(cfg.in_channels, cfg.widths, strict=False)
            ]
        )

        # Exchange units（HRNetの stage 内 fusion を複数回）
        self.units = nn.ModuleList(
            [ExchangeUnit(cfg.widths) for _ in range(cfg.num_units)]
        )
    ...
    def forward(self, feats: list[torch.Tensor]) -> torch.Tensor:
        ...
        xs = [lat(f) for lat, f in zip(self.lateral, feats, strict=False)]
...
if __name__ == "__main__":
    ...
    feats = [torch.randn(B, c, h, w) for (h, w), c in zip(shapes, C_ins, strict=False)]
