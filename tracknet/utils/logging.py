...
def _denormalize_image_tensor(
    t: torch.Tensor, mean: Sequence[float], std: Sequence[float]
) -> torch.Tensor:
...
def save_overlay_from_tensor(
    ...
    base = tensor_to_pil(img_t, denormalize=denormalize, mean=mean, std=std).convert(
        "RGBA"
    )
    x = hm.squeeze().detach().cpu()
    x = (x - x.min()) / (x.max() - x.min() + 1e-8)
