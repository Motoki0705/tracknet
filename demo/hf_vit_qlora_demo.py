...
def main():
    ...
    teacher = AutoModel.from_pretrained(
        args.model, torch_dtype=torch_dtype, device_map=None
    )
    teacher.to(dev())
    ...
    model_fp = AutoModel.from_pretrained(
        args.model, torch_dtype=torch_dtype, device_map=None
    )
    model_fp.to(dev())
    model_fp = apply_lora(
    ...
    model_q = convert_linear_to_int4(
        model_q, quant_type=args.quant_type, compute_dtype=compute_dtype
    )
    # attach LoRA
    model_q = apply_lora(
    ...
    print("\n=== Results ===")
    print(
        f"{'run':28s}  {'lat_ms':>8s}  {'samples/s':>10s}  {'max_mem(GB)':>11s}  {'train_loss':>10s}  {'val_loss':>8s}"
    )
    for r in results:
        print(
            f"{r.name:28s}  {fmt(r.lat_ms):>8s}  {fmt(r.samples_per_s):>10s}  {fmt(r.max_mem_gb):>11s}  {fmt(r.final_train_loss):>10s}  {fmt(r.final_val_loss):>8s}"
        )
