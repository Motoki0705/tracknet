...
@torch.no_grad()
def extract_features(model: nn.Module, pixel_values: torch.Tensor, kind: str = "cls") -> torch.Tensor:
    model.eval()
...
def run_train(model: nn.Module, train_loader: DataLoader, val_loader: DataLoader, cfg: TrainCfg, name="run") -> RunResult:
    ...
    for epoch in range(cfg.epochs):
        for xb, yb in train_loader:
            ...
            with torch.autocast(device_type=d.type, dtype=amp_dtype, enabled=torch.cuda.is_available()):
                if cfg.loss == "cosine":
                    out = model(pixel_values=xb, output_hidden_states=True).last_hidden_state[:,0,:]
                    if hasattr(model, "layernorm"): out = model.layernorm(out)
                else:
    ...
    lat_ms = dt * 1000.0; sps = xb.size(0) / dt
...
def main():
    ...
    ap.add_argument("--quant_mode", type=str, default="manual", choices=["manual","hf"])
    ...
    with torch.inference_mode():
        y_train = extract_features(teacher, pixel_values[: int(0.8 * len(pixel_values))], kind=args.target)
        y_val   = extract_features(teacher, pixel_values[int(0.8 * len(pixel_values)) :], kind=args.target)
    del teacher
    if torch.cuda.is_available(): torch.cuda.empty_cache()

    n = pixel_values.size(0); n_train = int(0.8 * n)
    ...
    res_a = run_train(model_fp, train_loader, val_loader,
                      TrainCfg(epochs=args.epochs, batch_size=args.batch_size, amp=args.amp, loss=args.loss),
    ...
    if args.quant_mode == "hf":
        if not BNB: raise SystemExit("bitsandbytes required for --quant_mode hf")
        ...
        skip_list = [s.strip() for s in args.skip_list.split(",")] if args.skip_list else []
        model_q = AutoModel.from_pretrained(args.model, dtype=amp_dtype, device_map=None)
        model_q = convert_linear_to_int4_manual(model_q, quant_type=args.quant_type,
                                                compute_dtype=(torch.bfloat16 if use_bf16 else torch.float16),
                                                skip_modules=skip_list)
    if not PEFT: raise SystemExit("peft is required")
    ...
    res_b = run_train(model_q, train_loader, val_loader,
                      TrainCfg(epochs=args.epochs, batch_size=args.batch_size, amp=args.amp, loss=args.loss),
    ...
    print("\n=== Results ===")
    print(f"{'run':45s}  {'lat_ms':>8s}  {'samples/s':>10s}  {'max_mem(GB)':>11s}  {'train_loss':>10s}  {'val_loss':>8s}")
    for r in [res_a, res_b]:
        print(f"{r.name:45s}  {fmt(r.lat_ms):>8s}  {fmt(r.samples_per_s):>10s}  {fmt(r.max_mem_gb):>11s}  {fmt(r.final_train_loss):>10s}  {fmt(r.final_val_loss):>8s}")
