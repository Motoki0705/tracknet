...
- **GIVEN** clip videos exist under `data/tracknet/<game>/<clip>/`
- **WHEN** I run `uv run python tracknet/tools/player_tracker.py --mode detect`
- **THEN** YOLOv8-based tracking results for every clip are saved to `outputs/tracking/person_tracks.json`.
